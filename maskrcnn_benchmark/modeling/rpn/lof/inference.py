...
class LOFPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        for i in range(N):
            ...
            per_box_cls = box_cls[i][per_candidate_inds]
            per_class = box_cls_id[i][per_candidate_inds]
            per_box_regression = box_regression[i][per_candidate_inds]
    ...
    def select_over_all_levels_fcos(self, boxlists):
        num_images = len(boxlists)
        results = []
        for i in range(num_images):
