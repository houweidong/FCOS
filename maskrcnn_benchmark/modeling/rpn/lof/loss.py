...
from maskrcnn_benchmark.layers import AELoss, AELossV2
...
class LOFLossComputationV2(object):
    """
    ...
    def __init__(self, cfg):
        ...
        self.num_lof = cfg.MODEL.LOF.NUM_LOF
    ...
    def compute_lof_tag_targets(self, lof_tag, labels, reg_targets, locations_to_gtis):
        ...
        locations_to_gtis_rep = locations_to_gtis[..., None, None].repeat((1, 1, self.num_lof, 1))

        lof_tag = torch.zeros_like(lof_tag)[..., None].repeat((1, 1, 1, max_num_bbox)).scatter_(
            -1, locations_to_gtis_rep, lof_tag[..., None])
        indicator = (torch.zeros_like(lof_tag).scatter_(-1, locations_to_gtis_rep, labels[..., None, None].repeat(
            (1, 1, self.num_lof, 1)).float()) > 0).float()
        lof_tag = lof_tag * indicator
        # to make shape (N, 5level, num_lof)
        centerness = self.compute_centerness_targets(reg_targets)[..., None].repeat((1, 1, self.num_lof))
        if self.avg_weight:
            centerness_box = centerness[..., None].repeat((1, 1, 1, max_num_bbox)) * indicator
        ...
        avg_lof_tag_gather = torch.gather(avg_lof_tag.repeat((1, len_pyramid_feature, 1, 1)), -1,
                                          locations_to_gtis_rep).squeeze(-1)
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, lof_tag, targets):
        ...
        for l in range(len(box_cls)):
            ...
            lof_tag_list.append(lof_tag[l].permute(0, 2, 3, 1).reshape(N, -1, self.num_lof))

        # shape (N * 5level, num_classes or 4)
...
class LOFLossComputation(object):
    ...
    def compute_lof_tag_targets(self, lof_tag, labels, reg_targets, locations_to_gtis):
        ...
        if self.avg_weight:
            ...
            avg_lof_tag = lof_tag.sum(1, keepdim=True) / num_bbox
        len_pyramid_feature = lof_tag.size(1)
        avg_lof_tag_gather = torch.gather(avg_lof_tag.repeat((1, len_pyramid_feature, 1)),
...
def make_lof_loss_evaluator(cfg):
    if cfg.MODEL.LOF.VERSION == 1:
        loss_evaluator = LOFLossComputation(cfg)
    elif cfg.MODEL.LOF.VERSION == 2:
        loss_evaluator = LOFLossComputationV2(cfg)
    else:
        raise Exception('not been supported version')
